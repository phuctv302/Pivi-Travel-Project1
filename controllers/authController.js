...
exports.login = catchAsync(async (req, res, next) => {
  // 1) Get password and email from body
  const { email, password } = req.body;

  // 2) Check if password or email are inputted
  if (!email || !password)
    return next(new AppError('Please provide us email or password!', 400));

  // 3) Check if user exists or password is correct
